...
jQuery(document).ready(function () {
    ...
    var speed = 500;
    var w = 300;
    var n = jq('#actors li').length;
    var numWidth = n * 18;
    var _left = (w - (numWidth + 26)) / 2;
    var c = 0;
    jq('#actors').width(w * n);
    jq('#actors li').each(function (i) {
        str += '<span></span>'
    });
    jq('#numInner').width(numWidth).html(str);
    jq('#imgPlays .mc').width(numWidth);
    jq('#imgPlays .num').css('left', _left);
    ...
    jq('.sell_ban #imgPlays .next_s').click(function () {
        slide(1)
    });
    jq('.sell_ban #imgPlays .prev_s').click(function () {
        slide(-1)
    });

    function slide(j) {
        if (jq('#actors').is(':animated') == false) {
            c += j;
            if (c != -1 && c != n) {
                jq('#actors').animate({
                        'marginLeft': -c * w + 'px'
    ...
    function fade(i) {
        if (jq('#actors').css('marginLeft') != -i * w + 'px') {
            jq('#actors').css('marginLeft', -i * w + 'px');
            jq('#actors').fadeOut(0,
                function () {
                    jq('#actors').fadeIn(500)
                })
        }
    }
