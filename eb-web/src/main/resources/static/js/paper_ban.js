...
jQuery(document).ready(function () {
    ...
    var speed = 500;
    var w = 228;
    var n = jq('#actorpa li').length;
    var numWidth = n * 18;
    var _left = (w - (numWidth + 26)) / 2;
    var c = 0;
    jq('#actorpa').width(w * n);
    jq('#actorpa li').each(function (i) {
        str += '<span></span>'
    });
    jq('#numInner').width(numWidth).html(str);
    jq('#imgPlaypa .mc').width(numWidth);
    jq('#imgPlaypa .num').css('left', _left);
    ...
    jq('.paper_ban #imgPlaypa .nextpa').click(function () {
        slide(1)
    });
    jq('.paper_ban #imgPlaypa .prevpa').click(function () {
        slide(-1)
    });

    function slide(j) {
        if (jq('#actorpa').is(':animated') == false) {
            c += j;
            if (c != -1 && c != n) {
                jq('#actorpa').animate({
                        'marginLeft': -c * w + 'px'
    ...
    function fade(i) {
        if (jq('#actorpa').css('marginLeft') != -i * w + 'px') {
            jq('#actorpa').css('marginLeft', -i * w + 'px');
            jq('#actorpa').fadeOut(0,
                function () {
                    jq('#actorpa').fadeIn(500)
                })
        }
    }
