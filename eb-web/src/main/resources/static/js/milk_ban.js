...
jQuery(document).ready(function () {
    ...
    var speed = 500;
    var w = 228;
    var n = jq('#actorm li').length;
    var numWidth = n * 18;
    var _left = (w - (numWidth + 26)) / 2;
    var c = 0;
    jq('#actorm').width(w * n);
    jq('#actorm li').each(function (i) {
        str += '<span></span>'
    });
    jq('#numInner').width(numWidth).html(str);
    jq('#imgPlaym .mc').width(numWidth);
    jq('#imgPlaym .num').css('left', _left);
    ...
    jq('.milk_ban #imgPlaym .nextm').click(function () {
        slide(1)
    });
    jq('.milk_ban #imgPlaym .prevm').click(function () {
        slide(-1)
    });

    function slide(j) {
        if (jq('#actorm').is(':animated') == false) {
            c += j;
            if (c != -1 && c != n) {
                jq('#actorm').animate({
                        'marginLeft': -c * w + 'px'
    ...
    function fade(i) {
        if (jq('#actorm').css('marginLeft') != -i * w + 'px') {
            jq('#actorm').css('marginLeft', -i * w + 'px');
            jq('#actorm').fadeOut(0,
                function () {
                    jq('#actorm').fadeIn(500)
                })
        }
    }
